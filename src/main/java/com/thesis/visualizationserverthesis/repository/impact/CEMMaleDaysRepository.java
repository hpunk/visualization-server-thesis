...
public interface CEMMaleDaysRepository extends JpaRepository<CEMMaleDays,Long> {
    ...
    @Query(value="select c.* from cem_male_victim_days c " +
    ...
    List<CEMMaleDays> searchCasesForImpact(@Param("filter") ImpactViolenceCasesFilter filter,@Param("startDate") LocalDate startDate,@Param("lastDate") LocalDate lastDate);
